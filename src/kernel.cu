...
void Boids::initSimulation(int N) {
  ...
  checkCUDAErrorWithLine("Allocate particle array indices failed");

  cudaMalloc((void**)&dev_particleGridIndices, N * sizeof(int));
  checkCUDAErrorWithLine("Allocate particle grid indices failed");

  cudaMalloc((void**)&dev_gridCellStartIndices, gridCellCount * sizeof(int));
  checkCUDAErrorWithLine("Allocate grid cell starts failed");

  cudaMalloc((void**)&dev_gridCellEndIndices, gridCellCount * sizeof(int));
  checkCUDAErrorWithLine("Allocate grid cell ends failed");
  ...
  checkCUDAErrorWithLine("Allocate sorted positions failed");

  cudaMalloc((void**)&dev_sortedVel, N * sizeof(glm::vec3));
  checkCUDAErrorWithLine("Allocate sorted velocities failed");
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
  ...
  if (cohesionCount > 0) {
      glm::vec3 center = positionSum / float(cohesionCount);
      cohesion = (center - selfPos) * rule1Scale;
  }

  glm::vec3 alignment(0.0f);

  if (alignmentCount > 0) {
      alignment = (velocitySum / float(alignmentCount)) * rule3Scale;
  }

  return cohesion + separation * rule2Scale + alignment;
...
__global__ void kernUpdateVelocityBruteForce(int N, glm::vec3 *pos,
  ...
  int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    glm::vec3 newVelocity = vel1[index] + change;
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 *pos, int *indices, int *gridIndices) {
    ...
    glm::vec3 relativePos =
        (pos[index] - gridMin) * inverseCellWidth;

    int x = static_cast<int>(floorf(relativePos.x));
    int y = static_cast<int>(floorf(relativePos.y));
    int z = static_cast<int>(floorf(relativePos.z));

    x = imax(0, imin(x, gridResolution - 1));
    y = imax(0, imin(y, gridResolution - 1));
    z = imax(0, imin(z, gridResolution - 1));
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  float searchRadius =
      fmaxf(rule1Distance, fmaxf(rule2Distance, rule3Distance));
  ...
  glm::vec3 minCell =
    (selfPos - glm::vec3(searchRadius) - gridMin) * inverseCellWidth;

  glm::vec3 maxCell =
      (selfPos + glm::vec3(searchRadius) - gridMin) * inverseCellWidth;

  int minX = imax(0, imin(static_cast<int>(floorf(minCell.x)), gridResolution - 1));
  int minY = imax(0, imin(static_cast<int>(floorf(minCell.y)), gridResolution - 1));
  int minZ = imax(0, imin(static_cast<int>(floorf(minCell.z)), gridResolution - 1));

  int maxX = imax(0, imin(static_cast<int>(floorf(maxCell.x)), gridResolution - 1));
  int maxY = imax(0, imin(static_cast<int>(floorf(maxCell.y)), gridResolution - 1));
  int maxZ = imax(0, imin(static_cast<int>(floorf(maxCell.z)), gridResolution - 1));
  ...
  glm::vec3 newVelocity =
      vel1[index] + cohesion + separation * rule2Scale + alignment;
...
__global__ void kernReorderBoids(
    ...
    sortedVel[index] = vel[originalIndex];
}
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  dim3 fullBlocksPerGrid(
    ...
    checkCUDAErrorWithLine("Identify cell ranges failed");
    ...
    checkCUDAErrorWithLine("Scattered velocity update failed");
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  kernReorderBoids<<<fullBlocksPerGrid, threadsPerBlock>>>(
      numObjects, dev_particleArrayIndices,
      dev_pos, dev_vel1, dev_sortedPos, dev_sortedVel);
  checkCUDAErrorWithLine("Reorder boids failed");
  ...
  checkCUDAErrorWithLine("Coherent velocity update failed");
